...
export default function Header() {
  function activeStyle({ isActive }: { isActive: boolean }): string {
    return isActive
      ? 'underline max-md:scale-110 max-md:translate-y-0.5 decoration-wavy decoration-2 underline-offset-4 decoration-accent/80'
  ...
  return (
    <header className={'flex w-full px-1 py-3 sm:px-6 sm:py-3'}>
      <nav className="flex w-full items-center justify-around align-middle font-zain text-[18px] font-extrabold text-primary/95 sm:text-[20px] sm:tracking-wide md:justify-end md:gap-8">
        <NavLink
          to={'/brawl'}
