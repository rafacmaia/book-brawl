...
const styles = {
  default: {
    ...
    input:
      'rounded-md text-base border-b-3 font-calistoga border-primary/85 placeholder-text/75 bg-blue-200 py-2.5 px-3 sm:p-2 sm:pl-3 shadow-lg disabled:opacity-65',
    button:
      'text-lg cursor-pointer rounded-md border-b-3 border-primary/70 bg-accent/80 pt-2.25 pb-1.5 text-center font-zain font-extrabold text-primary/90 shadow-md transition-all hover:scale-102 hover:bg-accent/85 active:scale-97 active:bg-accent/70 md:w-[8%] md:bg-accent/75 md:pt-1.75 md:pb-0.75 md:text-xl',
  },
  compact: {
    form: 'gap-2 flex w-full flex-col font-bold text-text md:gap-2.5',
    input:
      'rounded-lg text-sm border-b-3 placeholder-text/70  border-primary/85 bg-blue-200 py-2 px-3 shadow-lg md:text-base md:py-2.5 font-calistoga disabled:opacity-65',
    button:
  ...
  return (
    <>
      <form className={style.form} onKeyDown={handleKeyDown} onSubmit={handleSubmit}>
        ...
        />
        <button
          className={`${style.button} disabled:cursor-not-allowed disabled:opacity-80`}
          disabled={isLoading}
          title={'Add new book'}
          type="submit"
        >
